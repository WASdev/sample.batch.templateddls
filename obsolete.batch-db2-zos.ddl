...
-- operating platform for which the sample code is written. 
-- Notwithstanding anything to the contrary, IBM PROVIDES THE SAMPLE
-- SOURCE CODE ON AN "AS IS" BASIS AND IBM DISCLAIMS ALL WARRANTIES, 
-- EXPRESS OR IMPLIED, INCLUDING, BUT NOT LIMITED TO, ANY IMPLIED 
-- WARRANTIES OR CONDITIONS OF MERCHANTABILITY, SATISFACTORY QUALITY,
-- FITNESS FOR A PARTICULAR PURPOSE, TITLE, AND ANY WARRANTY OR 
-- CONDITION OF NON-INFRINGEMENT. IBM SHALL NOT BE LIABLE FOR ANY 
-- DIRECT, INDIRECT, INCIDENTAL, SPECIAL OR CONSEQUENTIAL DAMAGES 
-- ARISING OUT OF THE USE OR OPERATION OF THE SAMPLE SOURCE CODE. 
-- IBM HAS NO OBLIGATION TO PROVIDE MAINTENANCE, SUPPORT, UPDATES,
...
-- 3. Replace {tablePrefix} with table prefix.  
--       For example: PROD_JOBINSTANCEDATA
-- 4. Replace {databaseName} with the actual database name. 
--       For example: BATCHDB
-- 5. Replace {storegroup} with the actual storegroup name.  
--       For example: WQ1GSG
-- 6. Replace {bufferpool} with the  buffer pool name to be used 
--       for table spaces created within the database. For example: BP8
-- 7. Replace {bufferpoolindex} with the buffer pool name to be used for
--       the indexes created within the database. For example: BP9
-- 8. The following are tablespace defined for individual table, 
--       replace with a different value if requires.
...
--
-- 9. The following are table space defined for auxilary tables, 
--       replace with a different value if required
--
...
--
-- 10. The sample GRANT and REVOKE commands are issued to/from PUBLIC.  
--        Customize with desired auth id(s) as necessary.
...
CREATE LOB TABLESPACE JBELOBTS IN {databaseName}
   USING STOGROUP {storegroup} LOCKSIZE LOB;
...
CREATE TABLE {tablePrefix}JOBINSTANCEDATA(
   jobinstanceid BIGINT NOT NULL GENERATED ALWAYS AS IDENTITY 
      (START WITH 1, INCREMENT BY 1) 
      CONSTRAINT {tablePrefix}JOBINSTANCE_PK PRIMARY KEY,
   name       VARCHAR(512),
   apptag     VARCHAR(512),
   appname    VARCHAR(512),
...
CREATE TABLE {tablePrefix}EXECUTIONINSTANCEDATA(
   jobexecid BIGINT NOT NULL GENERATED ALWAYS AS IDENTITY 
      (START WITH 1, INCREMENT BY 1) 
      CONSTRAINT {tablePrefix}JOBEXECUTION_PK PRIMARY KEY,
   ...
   logpath       VARCHAR(512),
   CONSTRAINT {tablePrefix}JOBINST_JOBEXEC_FK 
      FOREIGN KEY (jobinstanceid) 
      REFERENCES {tablePrefix}JOBINSTANCEDATA (jobinstanceid) 
      ON DELETE CASCADE
...
CREATE TABLE {tablePrefix}STEPEXECUTIONINSTANCEDATA(
   stepexecid BIGINT NOT NULL GENERATED ALWAYS AS IDENTITY 
   (START WITH 1, INCREMENT BY 1) 
      CONSTRAINT {tablePrefix}STEPEXECUTION_PK PRIMARY KEY,
   jobexecid        BIGINT,
   batchstatus      VARCHAR(512),
   exitstatus       VARCHAR(512),
   ...
   CONSTRAINT {tablePrefix}JOBEXEC_STEPEXEC_FK FOREIGN KEY (jobexecid) 
      REFERENCES {tablePrefix}EXECUTIONINSTANCEDATA (jobexecid) 
      ON DELETE CASCADE
...
CREATE AUXILIARY TABLE {tablePrefix}STEPEXECUTIONINSTANCEDATABLOB
   IN {databaseName}.JBXLOBTS
      STORES {tablePrefix}STEPEXECUTIONINSTANCEDATA 
      COLUMN persistentData;

CREATE UNIQUE INDEX JBXLINDX ON
   {tablePrefix}STEPEXECUTIONINSTANCEDATABLOB 
      USING STOGROUP {storegroup}
      PRIQTY 720
      SECQTY -1
      BUFFERPOOL {bufferpoolindex};
...
CREATE TABLE {tablePrefix}JOBSTATUS (
   jobinstanceid    BIGINT NOT NULL 
      CONSTRAINT {tablePrefix}JOBSTATUS_PK PRIMARY KEY,
   batchstatus      VARCHAR(512),
   exitstatus       VARCHAR(512),
   latestjobexecid  BIGINT,
   currentstepid    VARCHAR(512),
   restarton        VARCHAR(512),
   CONSTRAINT {tablePrefix}JOBSTATUS_JOBINST_FK 
   FOREIGN KEY (jobinstanceid) 
   REFERENCES {tablePrefix}JOBINSTANCEDATA (jobinstanceid) 
   ON DELETE CASCADE,
   CONSTRAINT {tablePrefix}JOBSTATUS_JOBEXEC_FK 
      FOREIGN KEY (latestjobexecid) 
      REFERENCES {tablePrefix}EXECUTIONINSTANCEDATA (jobexecid) 
      ON DELETE CASCADE
...
CREATE TABLE {tablePrefix}STEPSTATUS (
   id  BIGINT NOT NULL 
   CONSTRAINT {tablePrefix}STEPSTATUS_PK PRIMARY KEY,
   obj BLOB(1M),
   CONSTRAINT {tablePrefix}STEPSTATUS_STEPEXEC_FK 
      FOREIGN KEY (id) 
      REFERENCES {tablePrefix}STEPEXECUTIONINSTANCEDATA (stepexecid) 
      ON DELETE CASCADE
...
CREATE TABLE {tablePrefix}CHECKPOINTDATA (
   id  VARCHAR(512) NOT NULL 
      CONSTRAINT {tablePrefix}CHECKPOINTDATA_PK PRIMARY KEY,
   obj BLOB(1M)
